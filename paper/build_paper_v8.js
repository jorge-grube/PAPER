// build_paper_v8.js — JF-style paper draft v8 FINAL FOR FEEDBACK
// Changes from v7:
//  - P1.2: Bootstrap count corrected to 5,000 (three locations: Sec II.F, Table III caption, Table VI caption)
//  - P1.3: Regime-Constrained CVaR values corrected (baseline: 0.522/0.519/29.2%; ZEW-swap: 0.519/0.517/27.0%)
//         after script 15 regime constraint States 2/3 inversion fix and rerun
//  - P1.4: Static CVaR Sharpe scope note added to Table VII caption; Sec V.A ref changed 0.528->0.551
//  - Updated intro para 6 and Sec V.B/IX prose to match corrected RC values
//  - P2.1: Table I note clarified (Brent and gold are not TR indices)
//  - P2.5: Introduction trimmed ~30% in paragraphs 5-8 (contributions/roadmap)
//  - P2.6: Nontechnical closing sentence added to Conclusion
//  - P2.7: Data licensing notice added to Section I (Sec I.B)
//  - P2.9: Sec V.A reference to Static CVaR at 10bps corrected to 0.551
//  - P3.7: ESI correlation clarified in Sec VII.B
//  - P3.8: Newey-West (1987) inline citation added in Sec II.F
//  - Output: paper_draft_JF_style_v8_FINAL_FOR_FEEDBACK.docx
// Run: node build_paper_v8.js
...
function buildTitlePage() {
  return [
    ...
    // Abstract: em dashes removed, math minuses removed
    new Paragraph({
      children: [run(
        'Using 26 years of weekly European multi-asset returns, we evaluate whether ' +
        'Hidden Markov Model regime conditioning improves out-of-sample risk-adjusted ' +
        'performance over static CVaR optimization. Static CVaR remains the most robust ' +
        'benchmark, achieving a Sharpe ratio of 0.530 over 2010 to 2026. Naive ' +
        'scenario-filtering raises annual portfolio turnover to 226%, eroding performance ' +
        'below benchmarks at any realistic transaction cost. Implementation-aware ' +
        'alternatives (turnover-penalized CVaR and regime-constrained weight bands) ' +
        'reduce cost drag substantially but do not surpass the static benchmark. Sovereign ' +
        'bond inclusion reduces drawdown but introduces material rate-shock exposure.'
      )],
      alignment: AlignmentType.JUSTIFIED,
      spacing: { before: 0, after: 0, line: 480, lineRule: 'auto' },
    }),
...
function buildIntroduction() {
  return [
    // Para 1
    bodyPara(
      'The proposition that financial markets cycle through distinct economic regimes is well-established ' +
      'in both theoretical and empirical finance. From Hamilton\'s (1989) seminal Hidden Markov Model for ' +
      'business-cycle identification to Ang and Bekaert\'s (2002) regime-switching asset return model, the ' +
      'literature documents that equity volatility, credit spreads, and correlation structures shift across ' +
      'identifiable market states. A natural question, and the one we address here, is whether this ' +
      'statistical regularity can be systematically exploited to improve out-of-sample portfolio performance ' +
      'through regime-conditional allocation.'
    ),
    blankLine(),
    // Para 2
    bodyPara(
      'The European multi-asset setting is a particularly demanding test environment for regime-conditional ' +
      'strategies. Europe has experienced four structurally distinct macro episodes within a single 26-year ' +
      'window: the post-dot-com normalization of 2002 to 2006, the Global Financial Crisis of 2008 to 2009, ' +
      'the European sovereign debt crisis of 2011 to 2012 with its unique peripheral spread dynamics, and ' +
      'the post-zero-interest-rate transition of 2022 in which ECB rate hikes imposed severe losses on ' +
      'long-duration bonds that had previously served as the universal risk-off instrument. Each episode ' +
      'rewarded a different defensive allocation: commodities and cash in 2008, government bonds in 2011, ' +
      'and commodities again in 2022. This structural instability in the cross-asset correlation regime makes ' +
      'Europe an informative laboratory for evaluating whether HMM-based conditioning can reliably identify ' +
      'and pre-position for such transitions in real time.'
    ),
    blankLine(),
    // Para 3
    bodyPara(
      'We study this question using ten risky assets spanning equities, commodities, real estate, and ' +
      'energy, over January 2000 to April 2026. Our primary research design combines Conditional ' +
      'Value-at-Risk (CVaR) portfolio optimization, a coherent, regulation-friendly risk measure ' +
      '(Rockafellar and Uryasev, 2000), with a four-state Gaussian Hidden Markov Model estimated on ' +
      'eight macro-financial features. The HMM generates strictly out-of-sample regime labels via an ' +
      'expanding walk-forward procedure, and we use these labels to condition the CVaR scenario set on ' +
      'the current market state (Regime CVaR-A) or to importance-weight scenarios proportionally to ' +
      'regime posteriors (Weighted CVaR).'
    ),
    blankLine(),
    // Para 4
    bodyPara(
      'A critical feature of our design is the strict out-of-sample discipline throughout. At each ' +
      'four-week rebalance step, the HMM is re-estimated from scratch on all available history up to ' +
      'that date, with no forward-looking information ever entering portfolio construction. This walk-forward ' +
      'expanding-window design contrasts with the partial in-sample evaluations common in earlier regime-' +
      'switching portfolio work and ensures that reported Sharpe ratios reflect genuine predictive content ' +
      'rather than in-sample fit (see Guidolin and Timmermann, 2007, for a discussion of this distinction). ' +
      'All transaction costs are applied at each rebalance using realized turnover, not the theoretical ' +
      'minimum implied by target weights.'
    ),
    blankLine(),
    // Para 5: main finding
    bodyPara(
      'Our central finding is that regime conditioning does not improve out-of-sample risk-adjusted ' +
      'performance relative to a static (regime-unconditional) CVaR benchmark. Over the 2010 to 2026 ' +
      'evaluation window (808 weekly observations), Static CVaR achieves an annualized Sharpe ratio of ' +
      '0.530 and a maximum drawdown of -25.3%. The naive regime-filtered strategy (Regime CVaR-A) ' +
      'achieves a Sharpe of only 0.365 with annual turnover of 226%, tenfold that of the static ' +
      'benchmark. At ten basis points of transaction costs, the net Sharpe of Regime CVaR-A falls to ' +
      '0.346 compared to 0.528 for Static CVaR.'
    ),
    blankLine(),
    // Para 6: implementation-aware variants
    bodyPara(
      'The high turnover is structural: regime transitions cause abrupt changes to the allowable ' +
      'scenario set, producing discontinuous jumps in the CVaR LP solution. Two implementation-aware ' +
      'alternatives recover much of the cost drag. An L1 turnover-penalized CVaR formulation reduces ' +
      'Regime CVaR-A turnover from 226% to 60% and recovers net Sharpe to 0.486. Regime-aware weight-band ' +
      'constraints achieve 0.519 net Sharpe with only 29% turnover. Neither surpasses Static CVaR.'
    ),
    blankLine(),
    // Para 7: FI expansion
    bodyPara(
      'A 14-asset FI-expanded universe adding sovereign bond total-return indices improves Panel A ' +
      'Sharpe by 0.034 and reduces maximum drawdown from -39.5% to -14.8%. The 2022 ECB rate-hiking ' +
      'cycle, however, inflicts -10% to -13% portfolio losses on the bond-heavy FI-expanded portfolio ' +
      'while the baseline, holding commodities and gold, avoids this shock. Bond inclusion changes the ' +
      'defensive allocation channel rather than uniformly improving all outcomes.'
    ),
    blankLine(),
    // Para 8: contributions and roadmap
    bodyPara(
      'Our findings contribute to three strands of literature. First, we extend regime-switching ' +
      'portfolio work by providing a rigorous, strictly OOS test of regime-conditional CVaR in a European ' +
      'multi-asset context (Guidolin and Timmermann, 2007; Ang and Bekaert, 2004). Second, we show that ' +
      'turnover can be internalized in the CVaR optimization, contributing to the literature on ' +
      'implementation frictions (Frazzini, Israel, and Moskowitz, 2015; Novy-Marx and Velikov, 2016). ' +
      'Third, we document the bond-commodity substitution dynamic in a CVaR-constrained universe, ' +
      'relevant for managers navigating the post-zero-rate environment. Sections I through IX cover ' +
      'data, methodology, regime characterization, baseline results, implementation variants, FI expansion, ' +
      'robustness, discussion, and conclusion in turn.'
    ),
    blankLine(),
  ];
}
...
function buildSectionVIII() {
  return [
    ...
    bodyPara(
      'Several limitations constrain the generalizability of these findings. The sample spans a ' +
      'specific European macroeconomic era characterized by declining rates from 2000 to 2022, ' +
      'followed by a single sharp rate-hiking episode. A longer or different sample might generate ' +
      'different HMM state structures and performance rankings. The four-state model is selected ' +
      'by BIC; alternative specifications might produce more or less actionable regime signals. ' +
      'All results are obtained on a single asset universe and rebalance frequency; generalization ' +
      'to other contexts requires further validation.'
    ),
    blankLine(),
    bodyPara(
      ...
      'The bootstrap confidence intervals for Sharpe ratios span approximately +/-0.4 units, meaning ' +
      'that point estimates of 0.365 (Regime CVaR-A) and 0.530 (Static CVaR) are not statistically ' +
      'distinguishable. The failure to find statistically significant outperformance is consistent ' +
      'with both "regime conditioning adds no value" and "the sample is too short to detect realistic ' +
      'Sharpe differences."'
    ),
    blankLine(),
    bodyPara(
      'Finally, the model abstracts from several market microstructure features that may materially ' +
      'affect real-world implementation costs. The 10 bps transaction cost assumption is a round-trip ' +
      'per unit of turnover; actual execution costs for institutional-size trades in index products ' +
      'and commodity futures may be higher or lower depending on market conditions, fund size, and ' +
      'trading venue. Bid-ask spreads on futures contracts are not modeled. Margin requirements, ' +
      'borrowing constraints, and liquidity management are abstracted away. In practice, a fund ' +
      'running a regime-conditioned CVaR strategy with 226% annualized turnover would also face ' +
      'operational complexity and capacity constraints that would further reduce the gross performance ' +
      'advantage relative to the static benchmark.'
    ),
    blankLine(),
  ];
}
...
Packer.toBuffer(doc).then(buf => {
  const fname = 'paper_draft_JF_style_v8_FINAL_FOR_FEEDBACK.docx';
